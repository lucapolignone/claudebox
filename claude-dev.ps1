...
<#
.SYNOPSIS
    claude-dev -- devcontainer Claude Code per la cartella corrente (auto-installante)

.DESCRIPTION
    Al primo avvio senza argomenti, o con il flag -Install, lo script si copia
    in un percorso nel PATH dell'utente e aggiunge un alias "claude-dev" al profilo
...
function Get-ContainerName { "claude-dev-$(Get-ProjectName)" }
...
function Invoke-Install {
    Write-Header "=== Installazione claude-dev ==="

    $installDir  = Get-InstallDir
    $destScript  = Join-Path $installDir 'claude-dev.ps1'
    ...
    # 3. Aggiunge alias "claude-dev" al profilo PowerShell
    ...
    $aliasLine = "function claude-dev { & '$destScript' @args }"
    $profileContent = Get-Content $profilePath -Raw -ErrorAction SilentlyContinue
    if ($profileContent -notlike "*claude-dev*") {
        Add-Content -Path $profilePath -Value "`n# claude-dev alias (aggiunto da claude-dev.ps1)`n$aliasLine"
        Write-Ok "Alias 'claude-dev' aggiunto al profilo: $profilePath"
    } else {
        Write-Info "Alias 'claude-dev' gia presente nel profilo"
    }
    ...
    Write-Host "  il comando 'claude-dev' da qualsiasi cartella." -ForegroundColor White
    Write-Host ""
    Write-Host "  Uso rapido:" -ForegroundColor White
    Write-Host "    cd C:\tuoi\progetti\mio-progetto" -ForegroundColor DarkGray
    Write-Host "    claude-dev init" -ForegroundColor DarkGray
    Write-Host "    claude-dev up" -ForegroundColor DarkGray
    Write-Host ""
}
...
function Invoke-Init {
    ...
    #   - volume Docker condiviso "claude-dev-shared-config" -> /home/node/.claude
...
"@
    ...
    Write-Host ""
    Write-Info "Prossimo passo: claude-dev up"
}
...
function Invoke-Up {
    ...
    if (-not (Test-Path ".devcontainer\devcontainer.json")) {
        Write-Err "Nessun .devcontainer trovato. Esegui prima: claude-dev init"
    }
    ...
    Write-Ok "Immagine pronta: claude-dev-img-$proj"
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "    5. Riesegui: claude-dev up" -ForegroundColor DarkGray
        Write-Host ""
        exit 1
    }
...
function Invoke-Update {
    ...
    Write-Info "Esegui 'claude-dev up' per ricostruire l'immagine con le modifiche."
}
...
function Invoke-Start {
    ...
    Write-Host "  |         claude-dev  --  avvio automatico              |" -ForegroundColor Blue
    Write-Host "  +======================================================+" -ForegroundColor Blue
    Write-Host ""
...
function Invoke-Shell {
    ...
    if (-not (Test-ContainerRunning)) {
        Write-Err "Container '$cname' non in esecuzione. Usa: claude-dev up"
    }
...
function Invoke-Destroy {
    ...
    Write-Host "  Il volume condiviso 'claude-dev-shared-config' NON viene rimosso (condiviso tra progetti)." -ForegroundColor DarkGray
    ...
    docker volume rm "claude-dev-${proj}-history" 2>$null | Out-Null
    Write-Ok "Volume history rimosso (se esisteva)"
    docker rmi "claude-dev-img-$proj" 2>$null | Out-Null
    ...
    Write-Host "    docker volume rm claude-dev-shared-config" -ForegroundColor DarkGray
}
